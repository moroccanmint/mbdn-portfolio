...
const Navbar = () => {
  ...
  return (
    <nav className='fixed top-0 left-0 w-full z-50'>
        <div className='flex items-center justify-between max-w-6xl mx-auto md:my-2 bg-stone-100/30 p-4 md:rounded-xl backdrop-blur-lg'>
            <div className='text-gray-800 font-semibold text-lg uppercase'>
                <a href='#'>Mark Buduan</a>
            </div>

            <div className='hidden md:flex space-x-8'>
                {LINKS.map((link, index) => (
                    <a href={link.href} key={index} onClick={handleLinkClick} className='text-gray-800 hover:text-stone-400 transition duration-300'>
                        {link.label}
                    </a>
                ))}
            </div>
